...
int spatialTrajOptimizer::bezierCurveGenerationEnforced( 
        const decomp_cvx_space::FlightCorridor &corridor,
        const MatrixXd &MQM_u,
        const MatrixXd &MQM_l,
        const MatrixXd &pos,
        ...
        const double maxVel, 
        const double maxAcc,
        const double relaxtol) 
{   
#define ENFORCE_VEL 1
#define ENFORCE_ACC 1
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                clow[m_idx]  = 0;
                cupp[m_idx]  = - pltp.planes[j](3) + relaxtol; // plane equation: ax + by + cz + K < 0 ==> ax + by + cz < -K
...
//  ######################################################
    ...
    cout << "ooqp resi:" << resid->residualNorm() << "|" << resid->dualityGap() << endl;
    // cout << "ooqp --resi:" << resid->calcresids(prob, vars) << endl;
